...
class RegionOverlay(QDialog):
    ...
    # Gut sichtbarer Auswahlrahmen:
    # außen schwarz für helle Hintergründe,
    # innen gelb für dunkle Elite-Menüs.
    ...
    def paintEvent(self, event) -> None:
        """
        Zeichnet einen gelben Auswahlrahmen mit schwarzer Außenkontur.

        Dadurch bleibt der Rahmen sowohl auf dunklen Elite-Menüs
        als auch auf hellen Bereichen der Galaxiekarte gut sichtbar.
        Der Ziehpunkt unten rechts wird weiß mit schwarzem Rand dargestellt.
        """

        painter = QPainter(self)
        painter.setRenderHint(QPainter.Antialiasing)

        # Schwarze Außenkontur.
        ...
        # Kleine gelbe Markierung im Ziehpunkt.
...
class ReferenceReviewDialog(QDialog):
    ...
    Der Wizard bewertet nur die Stabilität. Ob tatsächlich das
    richtige Motiv aufgenommen wurde, entscheidet der Benutzer
    anhand des direkten visuellen Vergleichs.
    """
    ...
    def __init__(
        ...
        *,
        ...
        self.setWindowTitle("Referenzaufnahme prüfen")
        self.resize(900, 520)
        self.setMinimumSize(760, 440)

        layout = QVBoxLayout(self)

        title = QLabel("Neue Referenzaufnahme prüfen")
        title.setStyleSheet("font-size: 20px; " "font-weight: bold;")
        title.setAlignment(Qt.AlignCenter)
        layout.addWidget(title)

        if stability_percent >= 97.0:
            stability_text = (
                f"Stabilität: {stability_percent:.2f} % – "
                "sehr gute und stabile Aufnahme."
            )
        elif stability_percent >= 93.0:
            stability_text = (
                f"Stabilität: {stability_percent:.2f} % – "
            ...
            stability_text = (
                f"Stabilität: {stability_percent:.2f} % – "
                "die Aufnahme ist wahrscheinlich nicht stabil."
            )

        stability_label = QLabel(stability_text)
        ...
        template_panel = self._create_image_panel(
            title="Asset-Vorlage",
            image_path=template_path,
            missing_text="Keine Asset-Vorlage gefunden.",
        )
        image_row.addWidget(template_panel)

        captured_panel = self._create_image_panel(
            title="Neue Aufnahme",
            image_path=captured_path,
            missing_text="Neue Aufnahme konnte nicht angezeigt werden.",
        )
        image_row.addWidget(captured_panel)

        layout.addLayout(image_row, 1)

        hint = QLabel(
            "Vergleiche beide Bilder kurz miteinander. "
            "Übernimm die Aufnahme nur, wenn Menüelement und "
            "Bildausschnitt zur Vorlage passen."
        )
...
class VisionWizardWindow(QMainWindow):
    def __init__(self) -> None:
        super().__init__()

        self.setWindowTitle("CTS Vision Wizard - " "Reference Setup & Calibration")
        self.resize(1180, 720)
        self.setMinimumSize(900, 620)

        self.output_dir = REFERENCES_DIR
        ...
        # Linke Seite: Referenzliste

        list_panel = QWidget()
        list_layout = QVBoxLayout(list_panel)

        list_title = QLabel("Referenzbilder")
        ...
        # Rechte Seite: Vorlage, Einstellungen
        # und Live-Vorschau
        ...
        self.choose_button = QPushButton("Speicherort wählen")
        self.choose_button.clicked.connect(self.choose_output)
    ...
    def refresh_reference_list(self) -> None:
        """
        Erstellt die nach Menüs gruppierte
        Referenzliste neu.
        """
        ...
        for (
            ...
            done, total_in_group = group_counts[group_name]

            group_item.setText(
                0,
                (f"{group_name} " f"({done}/{total_in_group})"),
            )
        ...
        self.progress_label.setText(f"{completed} / {total} " f"erstellt ({percent} %)")
    ...
    def reference_selection_changed(
        ...
        """
        Übernimmt den gewählten Namen und zeigt
        dessen Template.
        """
    ...
    def _get_window_position(window) -> tuple[int, int]:
        """
        Ermittelt die absolute Desktopposition des Elite-Fensters.

        Die im Wizard angezeigten X-/Y-Werte bleiben relativ zum
        Elite-Fenster. Nur der sichtbare Auswahlrahmen benötigt
        absolute Desktopkoordinaten.
        """
    ...
    def _get_elite_window_position(self) -> tuple[int, int]:
        """
        Sucht das Elite-Fenster und liefert dessen absolute
        Position auf dem gesamten virtuellen Desktop.
        """
    ...
    def _get_template_path(
        ...
        """
        Liefert den Pfad der Asset-Vorlage der gewählten Referenz.
        """
    ...
    def capture_reference(self) -> None:
        """
        Nimmt eine Referenz ohne sichtbaren Auswahlrahmen auf.

        Anschließend wird ein zweiter aktueller Ausschnitt aufgenommen
        und mit der neuen Referenz verglichen. So erkennt der Wizard
        instabile oder ungeeignete Aufnahmen sofort.
        """
        ...
        if not name:
            QMessageBox.warning(
                ...
                ("Bitte links ein Referenzbild " "auswählen."),
            )
            return
        ...
        if entry is None:
            QMessageBox.warning(
                self,
                "Unbekannte Referenz",
                (f"'{name}' ist nicht im " "Referenzkatalog enthalten."),
            )
            return

        output_path = self.output_dir / entry["filename"]
        temp_path = output_path.with_name(
            f".{output_path.stem}_aufnahme_test{output_path.suffix}"
        )
    ...
    def _capture_reference_first_image(
        ...
        *,
        ...
        """
        Speichert zunächst eine vorläufige Aufnahme.

        Die Referenz wird erst nach der Qualitätsprüfung endgültig
        übernommen.
        """
        ...
        # Eine zweite Aufnahme mit kleinem Abstand zeigt,
        # ob Animationen, Cursor oder andere Veränderungen
        # den gewählten Bereich instabil machen.
        QTimer.singleShot(
            300,
    ...
    def _validate_reference_capture(
        ...
        *,
        ...
        """
        Vergleicht die vorläufige Referenz mit einer zweiten
        aktuellen Aufnahme und entscheidet über das Speichern.
        """
        ...
        try:
            ...
            if should_save:
                if similarity >= 0.97:
                    ...
                    result_text = (
                        f"Stabilität: {percent:.2f} %\n\n"
                        "Aufnahme wurde nach visueller Prüfung übernommen."
                    )
                else:
                    result_text = (
                        f"Stabilität: {percent:.2f} %\n\n"
                        "Instabile Aufnahme wurde auf Wunsch übernommen."
                    )
            ...
            if should_save:
                ...
                self.result_label.setText(
                    f"{name}: {percent:.2f} % – bitte noch einmal"
                )
                ...
                QMessageBox.information(
                    ...
                    (
                        ...
                        "Richte den Ausschnitt bei Bedarf neu aus "
                        "und nimm die Referenz noch einmal auf."
                    ),
                )
    ...
    def _finish_reference_capture(
        ...
        *,
        ...
        """
        Räumt die vorläufige Aufnahme auf und zeigt den
        Auswahlrahmen wieder an.
        """
    ...
    def compare_reference(self) -> None:
        """
        Vergleicht den aktuellen Bildausschnitt
        ...
        try:
            ...
            reference_data = config["references"].get(name)

            if reference_data is None:
                raise KeyError("Keine Referenz mit dem Namen " f"'{name}' gefunden.")
    ...
    def show_region_overlay(self) -> None:
        """
        Zeigt einen verschiebbaren Auswahlrahmen.

        Die gespeicherten X-/Y-Werte sind relativ zum Elite-Fenster.
        Der Rahmen selbst benötigt dagegen absolute Desktopkoordinaten.
        """
    ...
    def update_region_position(
        ...
        """
        Übernimmt die absolute Rahmenposition und wandelt
        sie in Koordinaten relativ zum Elite-Fenster um.
        """
        ...
        self.x_input.setValue(max(0, relative_x))
        self.y_input.setValue(max(0, relative_y))

    def update_overlay_geometry(self) -> None:
        """
        Übernimmt X, Y, Breite und Höhe in den sichtbaren Rahmen.

        Die Eingabewerte sind relativ zum Elite-Fenster und werden
        für den Rahmen in absolute Desktopkoordinaten umgerechnet.
        """
